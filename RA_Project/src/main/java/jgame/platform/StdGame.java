...
/**
 * A basic framework for a game. It supports an animation and game timer, object
 * creation at fixed intervals, score, lives, levels, configurable keys. There
 * are title, start-level, next-level, death, and game-over sequences. Todo:
 * highscores, key configuration file and GUI.
 * 
 * <p>
 * To initialise this class, use the regular initEngine (from main), or
 ...
 * 
 * <p>
 * The class uses the following state machine, using JGEngine's state machine
 * mechanism:
 * 
 * <p>
 * <i>Title</i>: displays title screen. Transition to {StartLevel,StartGame}
 * when the key_startgame is pressed. Before the transition, initNewGame(),
 * defineLevel(), and initNewLife() are called.
 * 
 * <p>
 * <i>InGame</i>: game is playing. Transition to LifeLost when lifeLost() is
 ...
 * 
 * <p>
 * StdGame supports a set of standard game sequences, which are represented as
 * game states: StartLevel/StartGame, LevelDone, LifeLost, GameOver. These can
 * be configured so they add the InGame state to the sequence state (i.e. the
 * game is in both states simultaneously). This can be used to animate the game
 * while playing the corresponding sequence. This is off by default. The
 ...
 * 
 * <p>
 * <i>StartGame</i>: start game sequence is played. Transition to InGame after a
 * certain time has elapsed or the continuegame key is pressed.
 * 
 * <p>
 * <i>StartLevel</i>: start level sequence is played. Transition to InGame after
 * a certain time has elapsed or the continuegame key is pressed. Is always
 * active in combination with StartGame; it's just an indication that StartGame
 * is also a start of a new level.
 * 
 * <p>
 * <i>LevelDone</i>: next level sequence is played. Transition to
 ...
 * 
 * <p>
 * <i>LifeLost</i>: player has just died, a death sequence is played. Transition
 * to either GameOver or StartGame after a certain time has elapsed or the
 * continuegame key is pressed, dependent of whether there are lives left.
 * Before the transition to StartGame, decrementLives() and initNewLife are
 * called.
 * 
 * <p>
 * <i>GameOver</i>: game over sequence is played. Transition to Title after a
 * certain time or the continuegame key is pressed.
 * 
 * <p>
 * Built in are also game exit (through the key_quitgame, which is Escape by
...
public abstract class StdGame extends JGEngine {
	...
	private static final Logger log4j = LogManager.getLogger(StdGame.class
			.getName());
	/* settings */
	...
	public int key_left = KeyLeft, key_right = KeyRight, key_up = KeyUp,
			key_down = KeyDown;
	/** Key for moving diagonally, default = none. */
	// public int key_upleft=0, key_downleft=0,
	// key_upright=0, key_downright=0;
	/**
	 * Key for firing (in case there are no separate directional fire keys), JRE
	 * default is Z, MIDP default is Fire.
	 */
	...
	/**
	 * Game timer. Is reset to 0 at the beginning of each level, increments with
	 * gamespeed during InGame.
	 */
	public double gametime = 0;
	/**
	 * Sequence timer. Is reset to 0 at the start of the Title, Highscores,
	 * EnterHighscore, StartLevel, StartGame, LevelDone, LifeLost, GameOver
	 * sequences. Increments with gamespeed always. Can be used to time
	 * animations for these sequences.
	 */
	public double seqtimer = 0;
	/**
	 * Animation timer. Always increments with gamespeed. Can be used to time
	 * animations etc.
	 */
	...
	public int score = 0;
	/**
	 * Difficulty level; starts at 0 at beginning of game. Can be incremented
	 * each time a level is complete. Can be used to determine game difficulty
	 * settings throughout the game.
	 */
	public int level = 0;
	/**
	 * Game stage, which is usually the same as level, but typically goes on
	 * counting, while level may stop increasing at a certain value. Can be used
	 * to vary graphic sets, display stage number, etc.
	 */
	public int stage = 0;
	...
	/**
	 * The application configuration handler. Default is null, use initAppConfig
	 * to initialise it.
	 */
	...
	/**
	 * Highscore table, null (default) means not defined. Use setHighscores to
	 * define the table. If defined, the game will handle highscores by means of
	 * the states Highscores and EnterHighscore.
	 */
	...
	public void setHighscoreDisplay(int waittime, int showtime, JGFont font,
			JGColor color, String title, String entry, JGFont titlefont,
			JGColor titlecolor) {
	...
	/**
	 * Look for two arguments starting at index arg_ofs, and parse them as width
	 * and height. Returns (0.0) if there are no arguments. Prints a usage
	 * message otherwise.
	 */
	...
	public void initNewGame() {
		...
		/**
		 * @Dipanjan- added the logic to increase frame-rate based on game
		 *            status logging on 01/15/2018
		 */
		
		log4j.trace("Starting a new game");
		
		if (PresidentialRescue.control < 0.4)
		log4j.trace(" The current frame-rate is  " + getFrameRate());
		
		if (PresidentialRescue.control>0.4 && PresidentialRescue.game_counter == 0) {
			setFrameRate(getFrameRate(), 3);
			log4j.trace(" The current frame-rate is  " + getFrameRate());
			PresidentialRescue.game_counter++;

		} 
		else if (PresidentialRescue.control>0.4 && PresidentialRescue.game_counter > 0)
		{
			double framerate = getFrameRate();
			double newFramerate = framerate + 5;
			setFrameRate(newFramerate, 3);
			log4j.trace("The last frame-rate was "
					+ framerate);
			log4j.trace("The current frame-rate is  " + getFrameRate()
					+ " and score is " + score);
		}
	...
	/**
	 * Initialise play specifically after a new life is introduced (that is, at
	 * game start and after the player has died. This is typically used to
	 * reinitialise the player. If you want a specific initialization at both
	 * the beginning of the level or after the player death, use startInGame().
	...
	/**
	 * Code for incrementing a level before transition from LevelDone to InGame
	 * is made. Default is increment level and stage.
	 */
	...
	/**
	 * Call to make state transition to LifeLost. Is ignored when in another
	 * state than InGame or {InGame,StartLevel/StartGame}. After the LifeLost
	...
	public final void lifeLost() {
		if (!inGameState("InGame") || inGameState("LevelDone")
				|| inGameState("LifeLost") || inGameState("GameOver"))
			return;
		// System.err.println(
	...
	private void endLifeLost() {
		...
		if (lives <= 0) {
			gameOver();
			setFrameRate(getFrameRate(), 3);
			log4j.trace("This is a Game-over for all Life-lost");
			...
			if (startgame_ticks > 0) {
				...
				setGameState("InGame");
				/**
				 * @Dipanjan - Added this logging on 01/15/2018
				 */
				log4j.trace("Starting the game after losing a life");
				log4j.trace("The current frame-rate is " + getFrameRate()
	...
	/**
	 * Call to make state transition to LevelDone. Is ignored when state is not
	 * InGame or {Ingame,StartLevel/StartGame}. After the LevelDone sequence, it
	 * sets gametime to 0, calls incrementLevel and defineLevel, and goes to
	...
	/**
	 * Call to make straight transition to GameOver; is called automatically by
	 * lifeLost when appropriate. Is ignored when game state is not {InGame},
	 * {Ingame,Start*}, or LifeLost. Will go to Title after GameOver sequence.
	 */
	public final void gameOver() {
		// XXX hmm. we should check out these conditions
		if (inGameState("GameOver")
				|| (!inGameState("InGame") && !inGameState("LifeLost")))
			return;
	...
	private void gotoTitle() {
		...
		if (highscores != null && Highscore.findPos(highscores, score) >= 0) {
	 ...
	 * $JGAMEHOME/[classname].cfg, and save these in the StdGame object. Create
	 * your own AppConfig like this:
	 * 
	 * <pre>
	...
	public void initAppConfig() {
		appconfig = new AppConfig(getClass().getName().substring(
				...
				+ " settings", this, getConfigPath(getClass().getName()
				+ ".cfg"));
		...
		if (just_inited) {
			...
			if (highscores != null) {
				...
				if (inGameState("Title") && seqtimer >= highscore_waittime) {
					seqtimer = 0;
					setGameState("Highscores");
				} else if (inGameState("Highscores")
						&& seqtimer >= highscore_showtime) {
					seqtimer = 0;
					setGameState("Title");
				}
			}
	...
	public void doFrameEnterHighscore() {
		...
		if (key == KeyEnter) {
			highscores = Highscore.insert(highscores, new Highscore(score,
					playername));
			clearLastKey();
			clearKey(KeyEnter);
			saveHighscores();
		...
		if (key >= 32 && key < 127
				&& playername.length() < highscore_maxnamelen)
			playername += key;
	}

	/**
	 * Try to save highscores to default location,
	 * $HOMEDIR/.jgame/$CLASSNAME.hsc. Returns true on success, false on
	 * failure.
	 */
	...
	class QuitgameThread implements Runnable {
		...
		public void run() {
			...
			int opt = JOptionPane.showConfirmDialog(null, "Really quit?",
	...
	/**
	 * Initialisation at the start of the in-game action. This is a standard
	 * state transition function. Note that it is always called after StartLevel
	 * and LifeLost, even if startgame_ingame and lifelost_ingame are set.
	 * Default is do nothing.
	 */
	...
	/**
	 * Initialise start-level sequence. This is a standard state transition
	 * function. Default is do nothing.
	 */
	...
	/**
	 * Initialise start-game sequence. This is a standard state transition
	 * function. Default is do nothing.
	 */
	...
	/**
	 * Initialise next-level sequence. This is a standard state transition
	 * function. Default is do nothing.
	 */
	...
	/**
	 * Initialise death sequence. This is a standard state transition function.
	 * Default is do nothing.
	 */
	...
	/**
	 * Initialise game over sequence. This is a standard state transition
	 * function. Default is do nothing.
	 */
	...
	/**
	 * Initialise enter-highscore screen. This is a standard state transition
	 * function. Default is clear lastkey and set playername to the empty
	 * string.
	 */
	...
	/**
	 * Default paintFrame displays score at top left, lives at top right. When
	 * lives_img is set, it uses that image to display lives.
	 */
	...
	public void paintFrameTitle() {
		...
		drawString("Press " + getKeyDesc(key_startgame) + " to start",
				viewWidth() / 2, 6 * viewHeight() / 10, 0, title_font,
				title_color);
		drawString("Press " + getKeyDesc(key_gamesettings) + " for settings",
				viewWidth() / 2, 7 * viewHeight() / 10, 0, title_font,
				title_color);
	}
	...
	public void paintFramePaused() {
		...
		drawRect(viewWidth() / 20, 15 * viewHeight() / 36,
				18 * viewWidth() / 20, 5 * viewHeight() / 36
						+ (int) getFontHeight(title_font), true, false, false);
		...
		drawString("Press " + getKeyDesc(key_pausegame) + " to continue",
				viewWidth() / 2, 19 * viewHeight() / 36, 0, title_font,
				title_color);
	}

	/** Default displays "Level "+(stage+1). */
	public void paintFrameStartLevel() {
		drawString("Level " + (stage + 1), viewWidth() / 2,
				3 * viewHeight() / 5, 0, title_font, title_color);
	}

	/** Default displays "Start !". */
	public void paintFrameStartGame() {
		drawString("Start !", viewWidth() / 2, viewHeight() / 3, 0, title_font,
				title_color);
	}

	/** Default displays "Level Done !". */
	public void paintFrameLevelDone() {
		drawString("Level Done !", viewWidth() / 2, viewHeight() / 3, 0,
				title_font, title_color);
	}

	/** Default displays "Life Lost !". */
	public void paintFrameLifeLost() {
		drawString("Life Lost !", viewWidth() / 2, viewHeight() / 3, 0,
				title_font, title_color);
	}
	...
	/**
	 * Default displays highscore_entry, and the player's score and playername
	 * currently being entered.
	 */
	public void paintFrameEnterHighscore() {
		drawString(highscore_entry, viewWidth() / 2, viewHeight() / 3, 0,
				highscore_title_font, highscore_title_color);
		drawString("" + score, viewWidth() / 2, viewHeight() / 2, 0,
				highscore_font, highscore_color);
		drawString(playername + "|", viewWidth() / 2, 2 * viewHeight() / 3, 0,
				highscore_font, highscore_color);
	}

	/** Default displays the highscore list. Fields are not yet supported. */
	public void paintFrameHighscores() {
		drawString(highscore_title, viewWidth() / 2, viewHeight() / 7, 0,
				highscore_title_font, highscore_title_color);
		double yinc = 0.7 * viewHeight() / highscores.length;
		double ypos = 0.6 * viewHeight() - yinc * (highscores.length / 2.0);
		for (int i = 0; i < highscores.length; i++) {
			drawString("" + highscores[i].score, 0.35 * viewWidth(), ypos + i
					* yinc, 1, highscore_font, highscore_color);
			drawString(highscores[i].name, 0.6 * viewWidth(), ypos + i * yinc,
	...
	/**
	 * Draw a row of objects to indicate the value count. This is typically used
	 * to indicate lives left.
	 */
	...
	public void drawWavyString(String s, int x, int y, int align,
			int increment_x, double tmr, double amplitude,
			double pos_phaseshift, double timer_phaseshift, JGFont font,
			JGColor col) {
		setFont(font);
		setColor(col);
		if (align == 0) {
		...
		for (int i = 0; i < s.length(); i++)
			drawString(
					...
					x + i * increment_x,
					y
							+ (int) (amplitude * -Math.cos(Math.PI
									* (pos_phaseshift * i + tmr
											* timer_phaseshift))), 0);
	}
	...
	/**
	 * Get font for zooming text in and out. Note that tmr = 0 will start the
	 * font zooming in from the farthest position.
	 */
	public JGFont getZoomingFont(JGFont basejgfont, double tmr,
			double min_size_fac, double speed) {
		return new JGFont(basejgfont.name, basejgfont.style, basejgfont.size
	...
	public int posWalkForwards(int begin_pos, int end_pos, double tmr,
			int end_time, int standstill_pos, int standstill_time,
			int standstill_count) {
		if (tmr < standstill_time) {
			double step = (standstill_pos - begin_pos)
					/ (double) standstill_time;
			return begin_pos + (int) (tmr * step);
		} else if (tmr >= standstill_time
				&& tmr < standstill_time + standstill_count) {
			...
			double step = (end_pos - standstill_pos)
					/ (double) (end_time - beg2_time);
